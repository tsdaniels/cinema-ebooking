...
const MovieCard = ({ title, posterUrl, handleClick, id, rating }) => (
  <div className="relative overflow-hidden rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105 hover:shadow-2xl bg-black border border-red-900">
    ...
    <div className="p-4">
      ...
      <button
        ...
        className="mt-3 w-full bg-red-900 hover:bg-red-800 text-white py-2 px-4 rounded-md transition-colors duration-200 flex items-center justify-center"
      >
...
export default function SearchMovies() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      try {
        ...
        // Initially, categorize all movies based on status
  ...
  const handleSearch = (e) => {
    ...
    // Apply search and rating filter on all movies
    ...
    // After filtering, categorize movies into "Now Showing" and "Coming Soon"
      ...
      <div className="inline-flex rounded-lg bg-black border border-red-800 p-1">
        <button
          ...
          className={`px-6 py-2 rounded-md font-medium ${
            activeTab === "all" ? "bg-red-900 text-white" : "text-gray-400 hover:text-white"
          }`}
        >
          All Movies
        </button>
        <button
          ...
          className={`px-6 py-2 rounded-md font-medium ${
            activeTab === "now" ? "bg-red-900 text-white" : "text-gray-400 hover:text-white"
          }`}
        >
          Now Showing
        </button>
        <button
          ...
          className={`px-6 py-2 rounded-md font-medium ${
            activeTab === "soon" ? "bg-red-900 text-white" : "text-gray-400 hover:text-white"
          }`}
        >
          Coming Soon
        </button>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-12 text-white">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 justify-center mb-12 max-w-xl mx-auto">
          ...
          <select
            ...
            className="h-12 rounded-lg shadow-inner bg-black bg-opacity-80 border border-red-900 text-white px-4 focus:outline-none focus:ring-2 focus:ring-red-700"
          >
